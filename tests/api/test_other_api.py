...
from db_models.account_transaction_template import AccountTransactionTemplate, Base
...
class TestAccountTransactionTemplate:
    ...
    def test_accounts_transaction_template(self, db_session: Session):
        ...
        with allure.step("Создание тестовых данных в базе данных: счета Stan и Bob"):
            stan = AccountTransactionTemplate(user=f"Stan_{random.randint(1, 1000)}", balance=1000)
            bob = AccountTransactionTemplate(user=f"Bob_{random.randint(1, 1000)}", balance=500)
